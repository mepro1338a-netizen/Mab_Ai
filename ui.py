...
# Keep sidebar open permanently on page load
components.html(
    """
    <script>
    function openSidebarAlways() {
        try {
            const doc = window.parent.document;
            const sidebar = doc.querySelector('[data-testid="stSidebar"]');
            const collapsed = doc.querySelector('[data-testid="collapsedControl"]');

            if (collapsed && (!sidebar || sidebar.offsetWidth < 220)) {
                collapsed.click();
            }

            setTimeout(() => {
                const collapseButtons = doc.querySelectorAll('[data-testid="stSidebarCollapseButton"], button[title="Close sidebar"], button[aria-label="Close sidebar"]');
    ...
    openSidebarAlways();
    setTimeout(openSidebarAlways, 300);
    setTimeout(openSidebarAlways, 900);
    setTimeout(openSidebarAlways, 1600);
    </script>
    """,
    height=0,
    width=0,
...
/* Hide sidebar close/collapse button so it cannot be removed again */
...
/* If collapsed button appears, keep it usable but visually small */
