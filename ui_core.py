...
.stApp {{
    background:
        radial-gradient(circle at top left, rgba(168,85,247,.08), transparent 24%),
        radial-gradient(circle at top right, rgba(56,189,248,.10), transparent 24%),
        linear-gradient(180deg,#060812 0%,#0b1020 52%,#04060d 100%);
}}
...
.custom-topbar {{
    ...
    height:76px;
    z-index:999999;
    background:linear-gradient(90deg,rgba(14,6,24,.96),rgba(24,10,38,.96));
...
section[data-testid="stSidebar"] {{
    ...
    background:
        ...
        linear-gradient(180deg,#14051f 0%,#1b082b 38%,#100419 100%)!important;
...
section[data-testid="stSidebar"] .stCaption {{
    margin-top:.9rem;
    margin-bottom:.15rem;
    color:rgba(255,231,163,.72)!important;
    font-size:.72rem;
    font-weight:850;
    letter-spacing:.13em;
...
.mb-nav-icon-shell {{
    ...
    background:linear-gradient(135deg,rgba(255,231,163,.10),rgba(168,85,247,.10));
...
.stButton > button {{
    ...
    border:1px solid rgba(255,231,163,.16)!important;
    background:
        linear-gradient(135deg,rgba(28,10,42,.96),rgba(12,6,22,.98))!important;
    color:var(--mb-gold)!important;
    font-weight:900!important;
...
.stButton > button:hover {{
    ...
    background:
        linear-gradient(135deg,rgba(72,28,112,.92),rgba(20,8,34,.98))!important;
    box-shadow:
...
.mb-active-nav .mb-nav-icon-shell {{
    background:
        linear-gradient(135deg,rgba(255,231,163,.22),rgba(168,85,247,.24));
...
.mb-active-nav .stButton > button {{
    color:#ffffff!important;
    border-color:rgba(255,231,163,.42)!important;
    background:
        linear-gradient(135deg,rgba(126,34,206,.70),rgba(38,12,62,.98))!important;
    box-shadow:
...
.sidebar-user-card {{
    background:linear-gradient(180deg,rgba(20,8,34,.98),rgba(10,6,18,.98));
    border:1px solid rgba(255,255,255,.08);
    border-radius:24px;
    padding:18px;
    margin-top:20px;
    box-shadow:0 0 32px rgba(168,85,247,.10);
}}
...
def nav(label: str, page: str, icon: str) -> None:
    ...
    icon_col, button_col = st.columns([0.15, 0.85], gap="small")

    with icon_col:
        if path.exists():
            st.markdown('<div class="mb-nav-icon-shell">', unsafe_allow_html=True)
            st.image(str(path), width=24)
            st.markdown("</div>", unsafe_allow_html=True)
        else:
            st.markdown('<div class="mb-nav-icon-shell"></div>', unsafe_allow_html=True)
